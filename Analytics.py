...
# --- Title ---
st.markdown("<h1 style='text-align:center; color: #1E90FF;'>🌊 Marine Life Analytics Dashboard</h1>", unsafe_allow_html=True)
st.markdown("---")

# --- KPI Cards ---
with st.container():
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Species", len(df['Fish_Species'].unique()))
    col2.metric("Avg Fish Abundance", round(df['Fish_Abundance'].mean(),2))
    col3.metric("Avg Water Temp (°C)", round(df['Water_Temp_C'].mean(),2))
    col4.metric("Avg Dissolved Oxygen (mg/L)", round(df['Dissolved_Oxygen_mgL'].mean(),2))
